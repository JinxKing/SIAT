...
public class DataOutputStream extends FilterOutputStream implements DataOutput {
    ...
    void taintLeakByWriter(char[] data){
        String NEWTAG = "ACIDroid";
        String leakTag = "taintLeak: DataOutputStream-";
        int tag = Taint.getTaintCharArray(data);
        if(tag == Taint.intent_charArrayExtra){
            Taint.log(leakTag+"intent_charArrayExtra");
        }

    }

    void taintLeakByWriter(int data){
        String NEWTAG = "ACIDroid";
        String leakTag = "taintLeak: DataOutputStream-";
        int tag = Taint.getTaintInt(data);
        if(tag == Taint.intent_intExtra){
            Taint.log(leakTag+"intent_intExtra");
        }

    }

    void taintLeakByWriter(long data){
        String NEWTAG = "ACIDroid";
        String leakTag = "taintLeak: DataOutputStream-";
        int tag = Taint.getTaintLong(data);
        if(tag == Taint.intent_longExtra){
            Taint.log(leakTag+"intent_longExtra");
        }

    }

    void taintLeakByWriter(float data){
        String NEWTAG = "ACIDroid";
        String leakTag = "taintLeak: DataOutputStream-";
        int tag = Taint.getTaintFloat(data);
        if(tag == Taint.intent_floatExtra){
            Taint.log(leakTag+"intent_floatExtra");
        }

    }

    void taintLeakByWriter(String data){
        String NEWTAG = "ACIDroid";
        String leakTag = "taintLeak: DataOutputStream-";

        String res = Taint.intentTaintLeak(data);
        if(res!=null && res!=""){
            Taint.log(leakTag+res+"-"+data);
    ...
    void taintLeakByWriter(char data){
        String NEWTAG = "ACIDroid";
        String leakTag = "taintLeak: DataOutputStream-";
        int tag= Taint.getTaintChar(data);
        if(tag == Taint.intent_charExtra){
            Taint.log(leakTag+"intent_charExtra");
        }
